...
class ConfigCoordinator:
    ...
    def run_interactive_configuration(
        self, template: str = "openproject", prober_enabled: bool = True
    ) -> bool:
        """
        ...
        try:
            ...
            with Progress(
                ...
                task = progress.add_task(
                    "Initializing configuration system...", total=None
                )

                # Initialize config-manager with workspace paths
                if self._dev_mode:
                    # Development mode: let config-manager auto-detect
                    self.config_mgr = ConfigurationManager(template=template)
                else:
                ...
                result = self.config_mgr.run_interactive(
                    prober_enabled=prober_enabled, resume=False  # Start fresh
                )
    ...
    def load_existing_configuration(self, config_file: Path) -> bool:
        ...
        if not config_file.exists():
            self.console.print(
                f"[red]✗[/red] Configuration file not found: {config_file}\n"
            )
            return False
        ...
        try:
            ...
            if result.success:
                ...
                self.console.print(
                    f"[red]✗[/red] Failed to load configuration: {result.error}\n"
                )
                return False

        except ImportError as e:
            self.console.print(f"[red]✗[/red] Could not import config-manager: {e}\n")
            return False
        except Exception as e:
